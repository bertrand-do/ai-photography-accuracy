...
### Placeholder-and-Replace (the post-hoc lock)

**The measured problem with Lock-and-Outpaint on current editors.** Lock-and-Outpaint assumes the model honors frozen pixels. Measured twice on nano-banana-class editors, it does not: a "locked" region is treated as an *anchor*, not a freeze — the model re-renders everything it is handed. An eyewear frame composited into the canvas came back redrawn and ~25% undersized; a projected balustrade guide came back rebuilt at the model's own size. For a face with glasses, a re-render costs scale; for a perforation pattern it costs the entire product. So: **pixels you need protected must never enter the generator.**

**The flip.** Generate the world *first*, around a **placeholder** — a flat, featureless stand-in with the product's footprint — then lock *after*: composite the true product deterministically once generation is done. Same principle as Lock-and-Outpaint ("never redrawn = never wrong"), opposite order. The product pixels are never touched by a model.

**The recipe (all deterministic except step 1 and 3, which are cheap generations):**
1. **Placeholder scene** — prompt the product region as "a completely FLAT, FEATURELESS, uniform light-gray solid surface — no texture, no pattern, no reflections — like a blank primed placeholder". The model is free to be creative everywhere else; there is nothing accuracy-critical for it to destroy.
2. **Locate the placeholder** — color-mask + connected components → quad corners (min/max of x+y and x−y handle rotated/perspective quads). Two hard-won details: *erode* the mask a few pixels first or thin bright bridges (metal posts) merge every panel into one blob; use *separate masks* for sunlit vs shaded placeholders (warm light pushes "gray" off a single chroma rule). Manual reads off a labeled grid overlay are the fallback.
3. **Behind-content** — a second edit that removes the placeholder entirely so the model paints what is actually behind it. Required for any see-through or transparent product (holes, mesh, glass): the composite needs real pixels to show through.
4. **Place the product** — perspective-warp the product texture into the quad on a 2× supersampled canvas, then area-downscale. Without the supersample, circles collapse into squares and lozenges on the pixel grid.
5. **Lighting transfer** — multiply the placed product by the placeholder's own (blurred) luminance field normalized to its mean. The product inherits the scene's shading, falloff and shadows for free.
6. **Occlusion** — anything inside the placeholder region that is *not* the flat placeholder (posts, brackets, leaves, a handrail) is foreground. Detect it as deviation from a row-median background model of the region (occluders are thin, so the median sees through them), dilate, feather, and restore those original pixels on top. Skip this and the product floats over the railing.
7. **Scale Audit** on the finished image (see Judging).

**When to reach for it:** parametric or data-described products (perforated sheets, panels, labels, screens, signage, textiles with known repeats) where zero drift is achievable *by construction*; any product where a re-render is unacceptable; any see-through product.

**Geometry-first: the angle extension.** Lock-and-Outpaint needs an angle-matched source, which for a physical product is the Angle Bank. For a parametric product the angle bank is free and infinite — synthesize the source at any camera angle. Build the product in 3D, project it through a camera you choose into a schematic guide (flat placeholders + structure lines), and have the model dress the scene around the guide. The model will *not* keep the guide's exact quads (anchor again) — but a scene born from a consistent guide comes out **projectively consistent**, which a freeform AI scene at a strong angle is not. Then solve the geometry from the placeholders: line-fit each face's edges (average many corner reads — a 3D straight line is a straight image line, so residuals are read noise), vanishing points, **self-calibrate the focal length from the orthogonality of two faces** (`f² = −(v₁−c)·(v₂−c)` with the principal point at image center), metric aspect per face by ray–plane intersection, measured panel spans in plane space, one homography per face. Measured counter-example: a freeform corner-balcony scene had panel widths that contradicted its own depth cues (equal-panel fit residual floor ≈9% of face length under *any* aspect) — painted, not projected; unfixable by fitting. Envelope: moderate obliquity (roughly the angle of a real estate photo) is solid; extreme close-up wraparounds go to a true 3D render route.

**Subtractive variant — material from the model, geometry from the data.** When the product's *surface* is a rendering problem (brushed metal, gloss, glass, anodised colour) but its *geometry* is data (holes, cut-outs, print), don't overwrite the placeholder with a synthetic texture — that failed the realism bar: a procedural shader lit by a *matte* placeholder's luminance produces a matte result ("you inherit the placeholder's material, not just its light" — the sheets read as paper). Instead generate the product *blank* in its real material (an unperforated brushed-aluminium sheet, an unprinted glossy box) and let the deterministic layer act as a **cutter/mask**: subtract the holes, apply the exact print, add the bore rims. Surface = photoreal from the renderer; dimensions = exact from the data. Rule of thumb: **replace as little as accuracy requires.** Practical form: two placeholders — a flat matte one for detection and geometry (colour masks need it), then an in-place edit that turns the blanks into the material (edits preserve layout well); after the edit, re-match panels to faces by edge slope and position, not nearest centre — a shifted layout once perforated two background panels and left the real corner return solid.

**Fine patterns at distance.** A 40%-open pattern (Rv 8-12) at 4 m *is* mostly background; the sheet dissolves into see-through and any downsampling moiré reads as fabric. Two moves: supersample and filter down so the webs stay crisp, and frame closer — a photography decision the real photographer also makes.

**Solver fallbacks, logged.** (1) Equal-panel prior: railings and screens are built with equal panels — search the face aspect that equalises the mapped spans. (2) If no aspect satisfies it (residual stays high), the scene is painted, not projected: map each panel as its own spec-sized rectangle from its detected corners. (3) Weak perspective (near-parallel top/bottom lines or equal on-screen widths) → per-panel directly. Print which mode fired: the audit reader must know what was assumed.

**Audit at an angle:** rectify each face with its metric homography, then run the standard Scale Audit on the rectified patch. The vertical pitch against the known face height is anchor-true; the horizontal pitch inherits the calibrated aspect.

**Field case.** Metallideen24 perforated balustrade, Rg 10-26 (Ø10 mm / 26 mm pitch). Front view: pitch 26.3/25.8 mm (+1.0/−0.9%). Seifenblase 5/10/15/20 multi-size pattern with a 250 mm manufacturing rapport: all four diameters within tolerance, rapport recovered by autocorrelation at 250 mm (+0.0%). Prompted angle change of the finished image: +12.6% pitch drift, square holes, patchy density — FAIL. Geometry-first corner view: f self-calibrated at 921 px, front face 25.9/25.6 mm, side face 25.6/25.6 mm — PASS at ±2%.
...
- **Verify the verifier** — a measurement is only as good as its instrument on *this* image. Dark-blob hole detection under-reads holes that show bright sky through them; a threshold tuned on matte gray fails on lit metal. Whenever the material, lighting or resolution changes, re-run the audit on an image known to be exact (the composite is exact by construction) before trusting a verdict — and if the instrument fails, say the measurement is unavailable rather than reporting a pass.
...
8. **Periodic patterns: verify the rapport.** Manufactured patterns repeat (a stamping tool has a fixed tile). Autocorrelate the hole profile along the panel: the first strong peak is the rapport, comparable directly against the datasheet's repeat. A one-shot random scatter has no peak — and reads "too random" to the eye for exactly that reason.
