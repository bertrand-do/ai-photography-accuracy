...
**The house rule:** sharpest possible reference, one jump, three variations. The Sequential Pipeline is the peer technique for when the job needs control a single edit cannot hold (an angle that must be created first, a multi-product composition) and for model consistency across a series (where chaining from a consistent model source wins by design). The verdict is scoped to single-shot transformations; it never tested a series.
